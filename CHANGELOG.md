...
## v3.2.6 (2026-07-20)
### Fixed
* `runRegula()` hanging forever when called with an empty `paths` array. With no path arguments the regula CLI falls back to reading stdin, and `execFile` always hands the child an open stdin pipe, so the WASI process blocked on read and the promise never settled — neither resolving nor rejecting. The child's stdin is now closed explicitly and the call returns an empty result set. Passing input paths was never affected.
* `build-wasi.sh` now stamps `version.Version` and `version.GitCommit` via ldflags, matching the Makefile. The WASI build previously reported `unknown-version, build unknown-commit` while the native binary reported real values.
### Changed
* `test/test-wasi.js` honours a `WASI_RUNTIME` environment variable (default `node`) instead of hardcoding `node`, so the parity suite can be pointed at another runtime.
* `vendor/` is now gitignored; it is generated by `build-wasi.sh` via `go mod vendor`.
...
## v3.2.4 (2026-02-04)
### Added
* `index.d.ts` with TypeScript type definitions for `RegulaOptions` and `RegulaResult`
### Fixed
...
* maintained OPA v1.8.0 for compatibility with the existing policy-engine fork
### Changed
* go version: 1.24.7 → 1.24.11

## v3.2.3 (2026-02-04)
### Added
* documentation for all JS API options in the README

## v3.2.2 (2026-02-04)
### Added
* WASI build support (`build-wasi.sh`) producing `regula.wasm`
* npm package with a programmatic JS API (`runRegula`, `validate`) and a `regula` CLI wrapper
